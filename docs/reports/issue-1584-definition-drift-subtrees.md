...
| `LdPastingContext` and nontrivial-regime pasting wrappers | `MIPStarRE/LDT/Pasting/**` | Internal nontrivial-regime context.  The source theorem `Pasting.ldPasting` must remain unrestricted. | `thm:ld-pasting`, `thm:ld-pasting-in-induction-section`, `thm:main-induction`, and `thm:main-formal`. | #1601, with native children #1622 and #1627. |
