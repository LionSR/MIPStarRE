...
| Section 6 self-improvement packages: `SelfImprovementData` and `AnswerSelfImprovementData` | `MIPStarRE/LDT/MainInductionStep/Statements.lean` and `MIPStarRE/LDT/MainInductionStep/Theorems/SelfImprovementAssembly/` | Internal packages for recording the checked Section 9 output in the induction context.  The former slice-transport records have been retired; the active route uses the answer-valued carrier construction and then forgets to ordinary self-improvement data. | `thm:self-improvement-in-induction-section`, `thm:main-induction`, and `thm:main-formal`. | #1507; #1503 is historical. |
