...
| Former `OrthonormalizationSpectralObligation` | retired from `MIPStarRE/LDT/SelfImprovement/Theorems/OrthonormalizationSpectral.lean` | The spectral truncation route is now represented directly by `spectralTruncationStatement_of_sourceAlmostProjective`; no separate Section 9 obligation abbreviation remains. | `thm:self-improvement`, `thm:self-improvement-in-induction-section`, `thm:main-induction`, and `thm:main-formal`. | Retired under #1596; remaining repair and final-field work stays under #1515. |
...
| Left-lifted projectivization repair producers: `leftLiftedProjectivizationRepairProducer` and `leftLiftedProjectivizationRepair` | `MIPStarRE/LDT/MakingMeasurementsProjective/Producers.lean` | Construction theorems for the projectivization repair route.  The historical word `Producer` denotes a theorem producing the repair data, not a permissible new hypothesis of a source theorem.  The active `mainFormal` path now reroutes through the checked repaired line-169 estimate rather than a separate exact match-mass producer target. | `lem:orthonormalization-main-lemma`, `thm:self-improvement`, and the `thm:main-formal` completion route. | #1032. |
| Former `SpectralTruncationInput` | retired from `MIPStarRE/LDT/MakingMeasurementsProjective/Statements.lean` | Former input-wrapper form of the spectral-truncation step; replaced by direct `SpectralTruncationStatement` construction theorems. | `lem:projective-non-measurement`, `lem:orthonormalization-main-lemma`, `thm:self-improvement`, and `thm:main-formal`. | Retired under #1435. |
