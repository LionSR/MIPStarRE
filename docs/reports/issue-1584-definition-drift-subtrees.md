...
| Section 6 self-improvement packages: `SelfImprovementData` and `AnswerSelfImprovementData` | `MIPStarRE/LDT/MainInductionStep/Statements.lean` and `MIPStarRE/LDT/MainInductionStep/Theorems/SelfImprovementAssembly/` | Internal packages for recording the checked Section 9 output in the induction context.  The broad slice-transport route has been removed as a theorem-level obstruction; the narrow `SliceStrategyTransport` records remain as Lean-only interfaces, and the active route uses the answer-valued carrier construction and then forgets to ordinary self-improvement data. | `thm:self-improvement-in-induction-section`, `thm:main-induction`, and `thm:main-formal`. | #1507; #1503 is historical. |
...
| `HelperStrongSelfConsistencyBounds` | `MIPStarRE/LDT/SelfImprovement/Theorems/Results/HelperSSC/Core.lean` | Checked internal assembly record for helper strong self-consistency, below `thm:self-improvement`. | `thm:self-improvement`, `thm:self-improvement-in-induction-section`, `thm:main-induction`, and `thm:main-formal`. | #1596 completed the local route; #1515 is historical. |
| Former `OrthonormalizationSpectralObligation` | retired from `MIPStarRE/LDT/SelfImprovement/Theorems/OrthonormalizationSpectral.lean` | The spectral truncation route is represented directly by `spectralTruncationStatement_of_sourceAlmostProjective`; no separate Section 9 obligation abbreviation remains. | `thm:self-improvement`, `thm:self-improvement-in-induction-section`, `thm:main-induction`, and `thm:main-formal`. | Retired under #1596; #1515 is historical. |
...
| Former `SpectralTruncationInput` | retired from `MIPStarRE/LDT/MakingMeasurementsProjective/Statements.lean` | The spectral-truncation step is represented by `SpectralTruncationStatement` and direct construction theorems such as `spectralTruncationStatement_of_sourceAlmostProjective`, rather than by an `Input` wrapper. | `lem:projective-non-measurement`, `lem:orthonormalization-main-lemma`, `thm:self-improvement`, and `thm:main-formal`. | #1032. |
| Final-theorem role and completion witnesses: `retired same-carrier unsymmetrization consistency route`, `retired role-induction witness route`, `retired role-measurement witness route`, `retired diagonal orthonormalization witness route`, `retired diagonal completion witness route`, `retired final-transport witness route` | `MIPStarRE/LDT/Test/**` | Internal construction chain below `thm:main-formal`.  These records must be built from the pass condition, Section 6, and Section 5; they are not source hypotheses.  The unused `StrategySymmetrizationPackage` layer has been removed; the role-register symmetrization route now uses the direct construction and goodness lemmas. | `thm:main-formal`, with auxiliary blueprint nodes in `ch10_induction.tex` documenting the Step 3 and Step 6 construction route. | #1043, #1363, and #1566; final-theorem audit #1558. |
