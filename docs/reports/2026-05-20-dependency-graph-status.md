...
environments gave the same conclusion for the source theorem boundary in the
May 20 snapshot.  This paragraph is historical.  The current source entries
`thm:main-formal` and `thm:main-induction` link to corrected statements with
the documented \(k\ge400md\) correction, and `thm:main-formal` also assumes the
documented nonzero-sampling boundary \(0<k\).  The named source-range wrappers
from the May 20 snapshot have been retired rather than kept as live proof
...
| `prop:main-formal-source-k-range-boundary` | Historical public graph row. | The separate source-range and zero-sampling boundary node has been superseded by the corrected theorem statement. | The current final theorem statement includes \(k\ge400md\) and \(0<k\). | No action in this dated row. |
...
| `prop:main-induction-source-range-obligation` | Historical public graph row. | The source-range obligation node has been superseded by the corrected theorem statement. | The interval `md <= k < 400md` is a confirmed statement discrepancy, not a live proof branch under the corrected statement. | No action in this dated row. |
