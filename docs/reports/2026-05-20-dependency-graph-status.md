...
pasting invocation, is proof-complete.  A subsequent source-boundary cleanup
also proved the corrected two-space final theorem under the documented
large-\(k\) and nonzero-sampling statement corrections.  The public GitHub Pages
graph in this dated audit remains stale: it still contains retired successor
nodes and has not yet picked up the current proof-complete source-boundary
nodes or the proved Naimark display.
...
`MIPStarRE.LDT.Test.mainFormal_sourceConclusion`, whose saturated-error branch is
proved and whose remaining branch is the small-error obligation
`MIPStarRE.LDT.Test.mainFormal_sourceSmallErrorConclusion`, while the induction source
...
This paragraph is superseded by the 2026-05-23 cleanup.  The direct proof-hole
inventory described here has been discharged under the corrected statements:
the corrected large-`k` induction route is proof-complete, and the final
two-space source-boundary theorem is proof-complete after recording the
documented nonzero-sampling condition.  The old source-range wrapper names were
removed from the active Lean and blueprint surface; the interval
`params.m * params.d ≤ k < 400 * params.m * params.d` is now documented as the
confirmed factor-\(400\) statement correction rather than as an unfinished
branch of the formal theorem.  The former degree-zero family branch has also
been retired because the recursive predecessor hypothesis no longer assumes
`0 < d`.
...
| `thm:main-formal` | `references/ldt-paper/test_definition.tex:180-202` | Matches the printed formal theorem, including `k >= md`. | Linked to the source-faithful Lean statement `MIPStarRE.LDT.Test.mainFormal_sourceStatement`, which calls the named source-boundary wrapper `MIPStarRE.LDT.Test.mainFormal_sourceConclusion`; this wrapper proves the saturated-error branch and leaves `MIPStarRE.LDT.Test.mainFormal_sourceSmallErrorConclusion` as the remaining non-vacuous source boundary.  The current same-space, corrected large-`k` interface remains split out as `thm:main-formal-current-interface`. |
...
`thm:main-formal`, `prop:main-formal-source-reduction`,
