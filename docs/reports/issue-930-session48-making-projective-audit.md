...
The apparent line-169 issue is also already documented rather than silent. From state-dependent-distance closeness alone, Lean proves only the honest `triangleSub` consequences with loss `ζ₁+√ζ₂` (`ProjectivizationChain.lean:540-601`). The paper-tight exact `ζ₁` route is exposed as the additional invariant `ProjectivizationMatchMassMonotonicity`, which states that replacing `G^A` by `Q^A` and `G^B` by `Q^B` does not decrease the appropriate diagonal match mass (`ProjectivizationChain.lean:306-337`). Lean proves that canonical completion preserves this match mass (`ProjectivizationChain.lean:340-391`) and that, given the invariant, the exact Alice- and Bob-side consistency links follow (`ProjectivizationChain.lean:393-469`). The remaining producer-side obligation is the orthonormalization match-mass preservation field (`ProjectivizationChain.lean:472-535`), which #834 now records as part of the live Step-6 witness residual. Because that obligation is explicitly named and tracked, it is not an undocumented discrepancy for #930.
...
I did not open a new follow-up issue. The only live projectivization-related proof obligation found in this audit is the already tracked #834 Step-6 witness residual, whose remaining inputs include line-130 orthonormalization data and `OrthonormalizationMatchMassPreservation` witnesses. That is proof work, not documentation-only discrepancy work, and was intentionally left untouched.
