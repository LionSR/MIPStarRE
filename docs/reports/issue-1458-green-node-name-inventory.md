...
| `rem:lean-restricted-completion-total-mass` | Former `restrictSomeProjSubMeas_*_residual_le` total-mass declarations | Historical Lean-only restricted-completion total-mass route.  The generic residual-domination declarations have been removed from Lean; the current blueprint entry is ordinary explanatory prose rather than a green definition node. |
