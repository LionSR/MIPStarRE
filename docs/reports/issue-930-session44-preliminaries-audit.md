...
This scope avoids the active areas for #1000 (`MIPStarRE/LDT/Tactic/AvgCongr.lean`), #998 (`MIPStarRE/LDT/Commutativity/Transport/FullSlice*`), the then-current #997 optional distribution adapters, and the draft #889 Lean/Mathlib upgrade.  The #997 adapter files have since been retired as unused; this historical scope note only records what was active at the audit snapshot.  It also avoids the session 42 #930 slice: expansion/hypercube graph and global variance were not re-audited here.
