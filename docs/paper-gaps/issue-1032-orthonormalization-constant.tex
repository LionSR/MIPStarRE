...
This note is attached to \ghissue{1032}; the issue is resolved by proving the
source-aligned theorem statement, while the weaker completion-route
construction stays under a separate name.}
...
The source theorem is now proved under the declaration
\(\leanid{MIPStarRE.LDT.MakingMeasurementsProjective.orthonormalization}\).
The formal proof follows the paper's completion-to-measurement reduction and
then uses a sharp locality-preserving repair route that consumes the completed
measurement's bipartite self-consistency estimate at the correct \(2\zeta\)
scale, avoiding the extra factor introduced by the historical completion-route
proof.  The proved completion-route theorem remains available under the separate
name
\(\leanid{MIPStarRE.LDT.MakingMeasurementsProjective.orthonormalizationCompletionRoute}\).
