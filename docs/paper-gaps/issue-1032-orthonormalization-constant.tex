...
now proves this paper statement.
...
by proving the source-aligned theorem statement, while the heterogeneous
\path{lem:orthonormalization-main-lemma} proof remains a tracked obligation in
the same issue.  The weaker completion-route construction stays under a
separate name.}
...
This does not close the whole Section~5 proof obligation.  The declaration
\(\leanid{MIPStarRE.LDT.MakingMeasurementsProjective.orthonormalization}\)
still depends on the heterogeneous
\(\leanid{MIPStarRE.LDT.MakingMeasurementsProjective.orthonormalizationMainLemma}\),
whose proof remains a tracked \texttt{sorry} under \ghissue{1032}.  Thus the
constant part is repaired, but the source theorem is not yet a proof-complete
\leanok{} entry.
