...
is first completed to a measurement \(\widehat A\).  The completed measurement
has bipartite strong self-consistency error \(2\zeta\).  The paper then applies
the main orthonormalization lemma directly at this error level, obtaining
\[
  \widehat A_a \otimes I
    \approx_{84(2\zeta)^{1/4}}
  \widehat P_a \otimes I.
\]
Since \(84\cdot 2^{1/4} < 100\), this gives the advertised
\(100\zeta^{1/4}\) bound after discarding the completion outcome.
...
\(Q/X/\widehat X/P\) repair theorem.  The conversion doubles the scalar:
\[
