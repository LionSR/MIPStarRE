...
witness as a submeasurement inside
\leanid{SdpStatementWithSlackness.witness}. The associated witness record still
contains a field \path{primalTotalOperator} asserting $T$'s total is $I$, but
this is the paper's saturated-optimum conclusion, not a definition of primal
feasibility.
\footnote{See
...
\path{MIPStarRE/LDT/SelfImprovement/Theorems/Statements.lean}, lines 61--104.
...
construction, and slackness-carrying SDP statement all expose the primal
variable as a submeasurement. Saturation is recorded only for the selected
optimal witness and is extracted as a measurement by
\leanid{sdp_slackness_measurement}.
