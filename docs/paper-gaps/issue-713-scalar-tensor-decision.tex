...
register.\footnote{The declarations are \texttt{fullSliceABABAvg} and
\texttt{evaluatedSliceABABAvg} in
...
$\Lean{}$ development inserts a scalar-to-tensor bridge.\footnote{The
bridge lemma is
...
The full transport lemma\footnote{The declaration
\texttt{fullSlice\_scalar\_marginalize\_y} in the same file.}
assembles the chain:
\begin{align*}
  \bigl|
...
In the $\Lean{}$ public API, all four scalar averages are exposed
with the augmented bounds, while the tensor-form intermediates are
kept private, consistent with the decision to keep the public API
purely scalar.\footnote{The public scalar averages are
\texttt{fullSliceABAAvg}, \texttt{evaluatedSliceABAAvg},
...
mathematical discrepancy. The $\Lean{}$ formalization pays an extra
$2\sqrt{\zeta}$ to cross between the scalar public API and the tensor
form required by $\closenessOfIP$. This overhead is accounted for in
