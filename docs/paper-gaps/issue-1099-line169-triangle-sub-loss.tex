...
The blueprint remark on the Lean line-169 interface now records three distinct
routes:\footnote{See
\path{blueprint/src/chapter/ch04_projective.tex}, the remark
\path{rem:lean-line169-projectivization-match-mass}.}
the exact route through the construction-level invariant
\texttt{ProjectivizationMatchMassMonotonicity}, the generic
$\zeta_1 + \sqrt{\zeta_2}$ substitution route, and the local repaired route
$\zeta_1 + 10\zeta_1^{1/8}$ described above.
...
The separate exact-$\zeta_1$ formal interface remains the construction-level
match-mass monotonicity invariant discussed in issue~\ghissue{1099}.  In other
words, the formal development now cleanly distinguishes two questions:
