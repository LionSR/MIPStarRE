...
\doclink{MIPStarRE/LDT/MakingMeasurementsProjective/SpectralTruncation.lean}. The
downstream SelfImprovement wrapper is
\leanid{orthonormalizationSpectralObligation\_of\_sourceAlmostProjective} in
\doclink{MIPStarRE/LDT/SelfImprovement/Theorems/OrthonormalizationSpectral.lean}.}
These declarations prove the nontrivial paper branch under the hypotheses
$0 < \zeta$ and $\zeta \le 1/4$.  This matches the part of the cited argument in
which one may set $\delta = \sqrt{\zeta}$ and still satisfy
\path{eq:bound-on-delta}.  The remaining unconditional theorem-level packaging
still needs two additional branches: the trivial large-error case and the exact
$\zeta = 0$ endpoint described above.
