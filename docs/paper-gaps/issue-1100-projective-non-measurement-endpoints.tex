...
\begin{itemize}
  \item \textbf{Difficulty.} Medium.  The small-error truncation argument is
  the one printed in the paper, but an unconditional Lean theorem must also
  supply the exact endpoint $\zeta = 0$ and the large-error branch.
  \item \textbf{Estimated weight.} The endpoint and packaging branches require
  roughly \textbf{3--5 theorem interfaces} and \textbf{200--400 lines} of
  formal proof around spectral truncation and state-dependent distance.
  \item \textbf{Mathlib/project split.} Approximately \textbf{35\% Mathlib}
  and \textbf{65\% project-local}.  Mathlib supplies finite-dimensional
  spectral projections and order facts; the project supplies the LDT
  measurement interfaces and the tensor-positioned distance estimates.
  \item \textbf{Key Mathlib inputs.} Spectral calculus for Hermitian operators,
  positive semidefinite order, square-root inequalities, and finite matrix
  trace positivity.
\end{itemize}
...
\begin{enumerate}
  ...
  \leanid{spectralTruncationInput\_of\_sourceAlmostProjective} in
...
case, and
\leanid{spectralTruncationInput\_of\_sourceAlmostProjective} packages the
result as the input needed by the later Section~5 construction.}
