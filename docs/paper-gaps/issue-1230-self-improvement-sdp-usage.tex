...
\begin{enumerate}
  ...
  \[
    ...
    \quad\text{for all }g.
  \]
  Saturation should be obtained by completing primal slack into an outcome whose
  objective coefficient is positive semidefinite, without decreasing the
  objective.  Slackness should then be recovered from zero duality gap and
  ...
  this should be understood as an auxiliary conditional bridge, not as a
  consequence of the SDP dual constraints.  The faithful target is to relax that
  interface to the paper's $Z\succeq0$ and $Z\succeq A_g$ fields, or to replace
  the conditional bridge by the saturated-witness argument above.%
...
the paper and the conditional dominance bridge currently formalized in Lean.
...
The faithful completion of the paper route is still to prove or import strong
duality with complementary slackness, specialize it to the paper's block SDP,
produce a saturated optimal witness without assuming $I\preceq Z$, and connect
that witness to the self-improvement interface.
