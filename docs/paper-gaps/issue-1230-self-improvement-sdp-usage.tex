...
\begin{itemize}
  ...
  canonical strong-duality route was completed in \ghpr{1708}, and the obsolete
  dominance wrappers were removed in \ghpr{2422}.
...
The corresponding matrix-order lemma is therefore deliberately conditional.  The
declaration
\[
  \leanid{MIPStarRE.Quantum.eq_zero_of_nonneg_mul_eq_zero_of_one_le}
\]
proves the matrix-order fact under $I\preceq Z$.  Earlier versions of the
formalization packaged the resulting SDP data in declarations whose names ended
in \leanid{WithDominance}; those wrappers have now been removed from the live
SDP interface.  Their former existence did not prove that an arbitrary optimal
dual witness selected by Slater duality satisfies $I\preceq Z$.
...
Lean keeps the submeasurement boundary explicit, and \ghpr{1708} supplies the
