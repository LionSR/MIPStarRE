...
\[
  ...
  \qquad (g\in \polyfunc{m}{q}{d}).
  \tag{\texttt{self\_improvement.tex:66--68}}
\]
The paper's primal and dual programs are
\begin{align}
  \sup_{\{T_g\}_{g\in\polyfunc{m}{q}{d}}}
    &\quad \sum_{g\in\polyfunc{m}{q}{d}} \tr(T_g A_g) \\
  \text{subject to}
    &\quad T_g\ge 0 \qquad(g\in\polyfunc{m}{q}{d}), \notag\\
    &\quad \sum_{g\in\polyfunc{m}{q}{d}} T_g\le I, \notag
...
\begin{align}
  \inf_Z
    &\quad \tr(Z) \\
  \text{subject to}
    &\quad Z\ge A_g \qquad(g\in\polyfunc{m}{q}{d}). \notag
...
\[
  \sum_{g\in\polyfunc{m}{q}{d}} T_g = I,
  \qquad
  T_g Z = T_g A_g \quad(g\in\polyfunc{m}{q}{d}).
...
\begin{align}
  &\E_{\bu}\sum_h
    \bra{\psi} (T_h A^{\bu}_{h(\bu)})\otimes I\ket{\psi} \notag\\
  &\qquad = \sum_h \bra{\psi} (T_h A_h)\otimes I\ket{\psi}
   = \sum_h \bra{\psi} (T_h Z)\otimes I\ket{\psi} \notag\\
  &\qquad = \bra{\psi} Z\otimes I\ket{\psi}.
  \tag{\texttt{self\_improvement.tex:397--400}}
\end{align}
Here the middle expression uses the abbreviation
$A_h=\E_{\bu}A^{\bu}_{h(\bu)}$.  The same swap is used again in the strong
self-consistency proof, where it turns a sum over $T_h A_h$ into a term
involving $Z$.%
...
the boundedness conclusion is restated per slice in lines~477--484 and on
average over $\bx$ in lines~542--549.}
...
optimal conclusion is mathematically necessary.  If
$M=|\polyfunc{m}{q}{d}|$, the strict Slater witness displayed in the paper is
\[
  T_g = \frac{1}{2M}I,
  \qquad
  Z=2I.
...
\section{The blueprint and the current formal statement}

The informal blueprint follows the same separation.  Its SDP lemma repeats the
paper's primal constraint $\sum_gT_g\le I$, the dual constraint $Z\ge A_g$, and
the selected-optimum conclusions $\sum_gT_g=I$ and $T_gZ=T_gA_g$.%
\footnote{See \path{blueprint/src/chapter/ch07_self_improvement.tex}, lines
513--536.}
The blueprint also isolates the strict feasible witness before saturation:
\path{lem:sdp-uniform-feasible-witness} states that
$T_g=(2|\polyfunc{m}{q}{d}|)^{-1}I$ has total mass $(1/2)I$, and
\path{lem:sdp-matrix-feasible-bounds} records the same fact in the concrete
matrix layer together with the dual feasibility of $Z=2I$.  Finally,
\path{lem:sdp-matrix-slackness-output} records the post-strong-duality output:
a complete primal measurement, a feasible dual operator, equality of objective
values, and the equations $T_gZ=T_gA_g$.%
\footnote{These blueprint statements are in
\path{blueprint/src/chapter/ch07_self_improvement.tex}, lines~450--505.}
Thus the blueprint is faithful to the paper on the same boundary: the primal
program is a submeasurement program, while measurement total and slackness are
properties of an optimal witness supplied later.
...
This separation is faithful to both the paper and the blueprint, but it is not
yet a complete formal proof of the paper's SDP lemma.  The analytic
convex-optimization theorem which produces the slackness-carrying optimal pair
remains a formalization task.  It is tracked as \ghissue{1230}.  That task
should prove the strong-duality producer; it should not change the primal
feasible set from $\sum_gT_g\le I$ to $\sum_gT_g=I$.

\section{Conclusion}

The verdict is that there is no new mathematical discrepancy in how the paper
uses SDP.  The LDT paper uses SDP in one place: the self-improvement lemma.  The
SDP chooses a primal polynomial submeasurement $T$ and a dual positive operator
$Z$.  The rest of the proof uses their consequences, namely submeasurement
bounds for sandwiching, dual feasibility $Z\ge A_g$, the saturated optimum
...
chosen optimum, not a feasibility hypothesis.  The current blueprint and formal
statements record that boundary correctly and separate the reduced SDP wrapper
from the stronger slackness-carrying statement.  The remaining work is to
formalize the strong-duality and complementary-slackness producer itself; this
is a missing formal proof of the paper's SDP lemma, not a new discrepancy in the
source argument.
