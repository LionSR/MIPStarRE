...
corresponding state object in the formal development.  It is tracked by
\ghissue{933}.  The paper passages consulted here are in
...
\begin{itemize}
  \item \textbf{Difficulty.}  Low as a statement-alignment issue, but high
  blast radius if one tried to bundle normalization into the base state type.
  \item \textbf{Estimated weight.}  A local audit or citation repair is small;
  a type-level migration would be a major project-wide refactor.
  \item \textbf{Mathlib/project split.}  Mostly project-local.  Mathlib
  supplies positive semidefinite matrices, trace, and Cauchy--Schwarz
  infrastructure, while the project supplies the normalized-trace convention and
  the LDT expectation notation.
  \item \textbf{Key mathematical inputs.}  The paper's vector-state notation
  assumes $\langle\psi,\psi\rangle = 1$; the formal density-matrix object
  separates positivity from the predicate \leanid{QuantumState.IsNormalized}.
\end{itemize}
...
\path{MIPStarRE/LDT/Pasting/ComparisonLemmas/Common.lean}, lines 179--212.}
...
460--500, and the evaluated-side transport bound
\path{fullSlice_closenessOfIP_CAB_hEval_sqrt} in
\path{MIPStarRE/LDT/Commutativity/Main/Auxiliary/HEvalTransport.lean}.}
...
the missing-mass estimate, the spectral-truncation statement construction, and
the orthonormalization theorem all expose the normalized-state hypothesis.
...
\path{spectralTruncationStatement_of_sourceAlmostProjective} in
\path{MIPStarRE/LDT/MakingMeasurementsProjective/SpectralTruncation/ProjectiveNonMeasurement.lean};
\path{leftLiftedProjectivizationRepair} in
\path{MIPStarRE/LDT/MakingMeasurementsProjective/LocalityPreservingRepair.lean}; and
\path{orthonormalization} in
...
transport, switch-sandwich lemma, completion theorem, spectral truncation
statement, and orthonormalization theorem all link to declarations whose
signatures include an explicit normalized-state hypothesis.  Thus the blueprint
and Lean are compatible, but they place the convention at different levels: the
blueprint uses the standard mathematical meaning of $|\psi\rangle$ as a
normalized state, while Lean records the hypothesis as data because its base
state type contains all positive semidefinite matrices.
...
The normalization hypothesis is a faithful formal boundary condition whenever a
Lean theorem is meant to reproduce a paper-scale estimate for a state
$|\psi\rangle$.  The base \leanid{QuantumState} type should remain the positive
cone; source-facing estimates should instead expose
\leanid{QuantumState.IsNormalized} exactly at the points where the paper uses
the unit-vector convention.
