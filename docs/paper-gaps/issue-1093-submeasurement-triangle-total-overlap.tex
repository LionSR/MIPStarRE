...
\begin{itemize}
  \item \textbf{Difficulty.} Medium.  The issue is a missing
  submeasurement-total term in a triangle-substitution step, rather than a
  failure of the underlying state-dependent distance estimate.
  \item \textbf{Estimated weight.} One project-local transport estimate and its
  scalar absorption into the self-improvement final-fields construction.
  \item \textbf{Mathlib/project split.} The obstruction is project-local:
  Mathlib supplies the elementary order and square-root inequalities, while the
  additional term comes from the project's submeasurement consistency
  definitions.
  \item \textbf{Key mathematical inputs.} The measurement-valued triangle
  substitution lemma, the submeasurement total-overlap displacement, and the
  projective point-consistency transport in the self-improvement final-fields
  route.
\end{itemize}

\section{Key theorem forms for the projective point-consistency transport}
...
\doclink{MIPStarRE/LDT/Preliminaries/Triangles/Core.lean}.  The self-improvement
...
\doclink{MIPStarRE/LDT/SelfImprovement/Theorems/Results/BoundednessTransport/PointConsistencyLiteral.lean}.}
