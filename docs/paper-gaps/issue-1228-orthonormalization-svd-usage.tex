...
$X$ in \texttt{def:matrix-decomposition-Q}, records the SVD step in
\texttt{def:svd-of-X}, and defines $P$ from
$\widehat X=UI_{m\times d}V^\dagger$ in \texttt{def:projective-P}.%
...
\texttt{lem:X-squared}, $\widehat X\widehat X^\dagger=I$ in
\texttt{lem:X-hat-squared}, $X^\dagger\widehat X=\sqrt Q$ in
\texttt{lem:X-times-X-hat}, the squared-difference estimate in
\texttt{lem:squared-difference}, and the final $P$-versus-$Q$ estimate in
\texttt{lem:P-Q-approx}.%
...
The formal development mirrors the blueprint separation.  The object standing
for the paper's SVD stage is a reduced data package of downstream consequences,
not a record of all SVD factors.%
\footnote{The relevant formal names are \leanid{svdOfX} and
\leanid{QXPLayerData} in
\doclink{MIPStarRE/LDT/MakingMeasurementsProjective/QXPLayer/Core.lean},
lines~184--191.}
This package stores the $Q$-layer, the matrices $X$ and $\widehat X$, the
restatement $Q_a=X^\dagger T_aX$, projectivity of the $Q_a$, and the two
...
\footnote{This is the constructor
\leanid{QXPLayerData.ofQLayerAndSvdIdentities} in
\doclink{MIPStarRE/LDT/MakingMeasurementsProjective/QXPLayer/Core.lean},
lines~202--269.}
...
the unitary laws and $X=USV^\dagger$, then the coisometry identity for
$UI_{m\times d}V^\dagger$ and the raw mixed-product formula follow by matrix
algebra.  A separate positive-square-root uniqueness lemma turns the mixed
product into $\sqrt Q$ when the middle factor is positive and has square $Q$.%
\footnote{The relevant formal declarations are
...
$m\le d$, the construction produces a rectangular coisometry $\widehat X$ such
that
\[
  \widehat X\widehat X^\dagger=I,
  \qquad
...
\doclink{MIPStarRE/LDT/MakingMeasurementsProjective/QXPLayerIdentities.lean}.
The project-local dependency was tracked in \ghissue{1228}, following the older
SVD-interface work in \ghissue{1117} and \ghissue{652}.}
The downstream estimates consume only these primitive identities, just as the
paper's later proof does.%
\footnote{The downstream formal declarations are \leanid{xHatSquared},
\leanid{xTimesXHat}, \leanid{squaredDifference}, and \leanid{pQApprox} in
\doclink{MIPStarRE/LDT/MakingMeasurementsProjective/QXPLayerIdentities.lean}.}
...
SVD-stage wrapper, the $X/\widehat X/P$ data package, and the constructors which
turn supplied identities into that package are mostly wrappers.%
...
variants.}
They keep the paper's $Q\to X\to\widehat X\to P$ objects visible and discharge
bookkeeping such as $X^\dagger X=Q$ from $Q_a=X^\dagger T_aX$ and
$\sum_aT_a=I$.  They do not themselves prove a general SVD theorem.
...
\doclink{MIPStarRE/Quantum/FiniteHilbert.lean} is built from Mathlib's
