...
argument cannot yield the exact error $\zeta_1$. The present note isolates a
sharper local repair that was considered for the repaired line-169 route.  The
current \leanid{MIPStarRE.LDT.Test.mainFormal} assembly no longer uses this
route: it keeps the exact line-169 match-mass preservation as a named proof
obligation instead of introducing a repaired transport helper into the final
theorem pipeline.
...
to the generic corrected route.  It remains useful mathematical documentation,
but it is not part of the active
\leanid{MIPStarRE.LDT.Test.mainFormal} proof route.
...
separate bridge or residual hypothesis.  The final transport that should be
used once the projective-completion residual has been constructed is factored
through
