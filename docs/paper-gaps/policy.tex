...
Most notes should then have four mathematical sections: the source assertion,
the mechanism being checked, the key theorem forms to develop or use, and the
blueprint/formal comparison, followed by the conclusion.  Use more sections only
when a counterexample or long derivation genuinely needs room.
...
section ``Form of the Note'' below specifies where such traceability data
belongs; the body should read as mathematical prose, not as a log of what
commands were run.

If the verdict is that there is no mathematical discrepancy, say so plainly and
stop.  Do not turn the note into a broad audit report.  If the only remaining
work is an implementation task, name it briefly and link the issue or pull
request in a footnote.
