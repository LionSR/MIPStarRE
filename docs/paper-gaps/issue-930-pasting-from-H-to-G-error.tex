...
Throughout this note, $k$ is the number of pasting, or sampling, stages;
$m$ is the number of variables in each slice; $d$ is the individual-degree
bound; and $q$ is the field size. The scalars $\gamma$ and $\zeta$ are,
respectively, the consistency and self-consistency errors carried by the
surrounding induction. The operator $G$ is the average slice submeasurement, and
$I-G$ is its complement in the Bernoulli polynomial that is compared with the
all-outcomes expansion. The symbol $\widehat H$ denotes the sandwiched
completed-slice product summed over all completed outcomes. Finally, $\nu_4$ is
the commutation error of \path{lem:commute-g-half-sandwich}, and $\nu_8$ is the
from-$H$-to-$G$ error parameter in the lemma under discussion.

The lemma compares the all-outcomes expansion of the pasted submeasurement with
a Bernoulli polynomial in $G$ and $I-G$. It states
\[
  \nu_8
