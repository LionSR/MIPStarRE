...
(blueprint \texttt{prop:triangle-sub}, bound \(\delta+\sqrt\eps\)),
\leanid{simeqTriangleInequality}~$\to$~\leanid{simeqTriangleInequality\_heterogeneous}
(\texttt{prop:simeq-triangle-inequality}, bound \(\eps+2\sqrt{\delta+\gamma}\)),
\leanid{simeqToApprox}~$\to$~\leanid{simeqToApprox\_heterogeneous}
(\texttt{prop:simeq-to-approx} forward, \(\simeq_\delta\Rightarrow\approx_{2\delta}\)),
...
(\texttt{prop:simeq-data-processing}, bound \(\delta\)).
All five \texttt{\_heterogeneous} forms are \texttt{sorry}-free.
...
\leanid{strategy.state : QuantumState (\(\iota\times\iota\))}.  The global proof never needs
...
\(\simeq_\delta\).%
\footnote{Related: \ghissue{2337} (same-space cleanup of the \texttt{mainFormal}
