...
including the term involving \(\eps\).

\section{Comparison with the blueprint and formalization}

The formalization currently keeps the printed theorem visible as a
source-shaped statement and isolates this boundary as the declaration
\leanid{mainFormal\_sourceZeroKBoundaryObligation}.  The scalar equality
\leanid{mainFormalError\_zero\_k} records the formal calculation that the
advertised error is zero at \(k=0\).

The nonzero branch is of a different nature.  Once \(0<k\) is available, the
current role-register and scalar-cascade route gives the final source-shaped
conclusion, up to the inherited Section~6 source-range issue documented in
\path{docs/paper-gaps/issue-906-main-formal-k-bound.tex}.  The zero-sampling
boundary is therefore not another request for a package or bridge input; it is
a statement-level boundary case.
...
does not vanish at \(k=0\).  The existing Lean declaration
\leanid{mainFormal\_sourceZeroKBoundaryObligation} should not be treated as a
proof obligation dischargeable by further packaging.
