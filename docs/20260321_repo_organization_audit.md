...
- Before this cleanup, the generic `docs/roadmap.md` name was the single most misleading filename in the repo. Renaming it to `docs/20260308_strict_2111_roadmap.md` makes the legacy scope clearer, but the underlying note is still legacy 2111 planning material rather than active Paper2009LDT guidance.
...
- The same stale-pointer problem also showed up in:
  ...
  - `docs/20260308_strict_2111_effort_estimate.md`
  - `docs/20260308_lean_quantuminfo_reuse_2111.md`
...
- `docs/20260320_ldt_source_map.md` is no longer fully synchronized with the current active blueprint. The clearest example is the self-improvement split. The source-map note still says the blueprint keeps a single `thm:self-improvement` node carrying both Lean links, but the current blueprint now has separate nodes in:
...
1. **Separate active and legacy docs explicitly.**
   The cleanest move would be to keep `docs/` for active-track material and move 2111 notes into something like `docs/legacy/2111/` or `archive/2111/`. Renaming `docs/roadmap.md` to `docs/20260308_strict_2111_roadmap.md` is the first obvious step, but the larger active/legacy split would still improve navigation.
...
6. **Sync or retire the active LDT support docs that have drifted.**
   In particular:
   - `docs/20260320_ldt_source_map.md` should match the current blueprint state,
   - `docs/20260320_ldt_blueprint_dependency_review.md` should either be updated or clearly marked as a historical review with some findings already resolved.
