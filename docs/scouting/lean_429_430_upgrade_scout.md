...
These are not direct replacements for additive-character orthogonality, but they improve the finite-field/Galois-field spine around Frobenius, splitting fields, and finite-field extensions. Revisit `MIPStarRE/LDT/Preliminaries/FiniteFields.lean` and the scouting reports `stream_a_finite_fields.md`, `stream_a_faithfulness.md`, and `stream_b_polynomials.md` after a 4.29 upgrade.
