...
| `scripts/blueprint_axiom_audit_needed.py` | Path and diff classifier deciding when `blueprint-sync.yml` must run the Lean build and `\leanok` axiom audit. | `scripts/tests/test_blueprint_axiom_audit_needed.py` | `.github/workflows/blueprint-sync.yml` |
