...
| `SelfImprovementData.SliceStrategyTransport` | `MIPStarRE/LDT/MainInductionStep/Theorems/SelfImprovementAssembly/Core.lean` | Internal transport record for concrete ordinary slice strategies.  It records state and measurement identifications needed to run Section 9 on the slices; it is not a source hypothesis of `thm:main-induction`. | Ordinary Section 6 self-improvement assembly.  The answer-valued successor route now uses `AnswerSelfImprovementData.ofAnswerCarrier` rather than an ordinary-realization transport record. |
