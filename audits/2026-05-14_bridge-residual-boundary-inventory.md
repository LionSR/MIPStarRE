...
accounted for by explicit slice-boundedness hypotheses and the scalar-regime
token `CascadeHypotheses`.
...
|---|---|---|
| Explicit slice-boundedness hypotheses | Faithful boundary input | Encode the boundedness item in `references/ldt-paper/commutativity-G.tex:29-36` and `references/ldt-paper/ld-pasting.tex:28-35`: positive slice witnesses, the averaged residual bound, and domination by the averaged point operator.  No additional domination-target identification bridge is exposed. |
...
(`CascadeHypotheses` and the explicit slice-boundedness hypotheses) or internal
