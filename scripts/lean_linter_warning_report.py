...
The weekly linter-warning sweep and the manual auto-fix wrapper share this
parser so they agree on what counts as actionable linter debt.  The parser is
intentionally conservative: it records Lean warning lines and classifies them
from the adjacent `set_option linter.<name> false` hint when Lean prints one,
falling back to a small list of known linter names in the warning message.
"""
...
def write_reports(
    *,
    ...
    """Parse `log_path` and write requested report files."""
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--github-summary",
        type=Path,
        help="Optional GITHUB_STEP_SUMMARY file to append a Markdown summary to",
    )
    parser.add_argument(
        "--count-output",
        type=Path,
        help="Optional GITHUB_OUTPUT file to append warning_count/actionable_warnings to",
    )
