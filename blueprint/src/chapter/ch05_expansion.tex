...
\criticize{Chapter~\ref{chap:expansion} is still semantically thin on the Lean side. In `Section7ExpansionHypercubeGraph.lean`, `adjacency` and `laplacian` are only named placeholder operators, and `localVariance` / `globalVariance` are still defined as `0`, so the spectral package has not yet been encoded mathematically.}{4}{5}
...
\end{lemma}
\begin{proof}
  The orthonormality is the finite-field Fourier orthogonality relation. The eigenvalue computation follows by substituting the edge-sampling rule into $K\ket{\varphi_\alpha}$ and averaging over the rerandomized coordinate.
\end{proof}
