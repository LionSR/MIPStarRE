...
\criticize{`Section12Pasting.lean` now makes the main theorem and many intermediate outputs explicit packages instead of collapsing them all to `True`, but the heavy algebraic core of the chapter is still abstract. The sandwiched-product chain, the Bernoulli-polynomial rewrite, and the matrix tail bound are still only named interfaces rather than implemented operator arguments.}{4}{5}

\begin{theorem}[Pasting]\label{thm:ld-pasting}
  \lean{MIPStarRE.Paper2009LDT.Section6MainInductionStep.ldPastingInInductionSection, MIPStarRE.Paper2009LDT.Section12Pasting.ldPasting}
...
\begin{lemma}[Pasted submeasurement]\label{lem:ld-pasting-sub-measurement}
  \lean{MIPStarRE.Paper2009LDT.Section12Pasting.ldPastingSubMeasurement}
  \uses{def:polymeasurements, def:simeq}
...
\begin{lemma}[Distinct tuples versus independent tuples]\label{prop:ld-dnoteq}
  \lean{MIPStarRE.Paper2009LDT.Section12Pasting.ldDnoteq}
...
\begin{lemma}[A scalar inequality for the first construction]\label{lem:looks-easy-but-took-me-a-while}
  \lean{MIPStarRE.Paper2009LDT.Section12Pasting.looksEasyButTookMeAWhile}
...
\begin{lemma}[Strong self-consistency of the complete part]\label{lem:g-complete-self-consistency}
  \lean{MIPStarRE.Paper2009LDT.Section12Pasting.gCompleteSelfConsistency}
...
\begin{lemma}[Strong self-consistency of the incomplete part]\label{cor:g-bot-self-consistency}
  \lean{MIPStarRE.Paper2009LDT.Section12Pasting.gBotSelfConsistency}
...
\begin{lemma}[Commutativity with $G_g^x$ implies commutativity with $G^x$]\label{lem:commutativity-switcheroo}
  \lean{MIPStarRE.Paper2009LDT.Section12Pasting.commutativitySwitcheroo}
...
\begin{lemma}[Commutativity of the complete part]\label{cor:commuting-with-G-complete}
  \lean{MIPStarRE.Paper2009LDT.Section12Pasting.commutingWithGComplete}
...
\begin{lemma}[Commutativity of the incomplete part]\label{cor:commuting-with-G-incomplete}
  \lean{MIPStarRE.Paper2009LDT.Section12Pasting.commutingWithGIncomplete}
...
\begin{lemma}[Strong self-consistency and commutation of $\widehat G$]\label{cor:G-hat-facts}
  \lean{MIPStarRE.Paper2009LDT.Section12Pasting.gHatFacts}
...
\begin{lemma}[Commuting past multiple completed measurements]\label{lem:commute-g-half-sandwich}
  \lean{MIPStarRE.Paper2009LDT.Section12Pasting.commuteGHalfSandwich}
...
\begin{lemma}[Consistency of the sandwiched measurement with one line value]\label{lem:ld-sandwich-line-one-point}
  \lean{MIPStarRE.Paper2009LDT.Section12Pasting.ldSandwichLineOnePoint}
...
\begin{lemma}[Consistency of the pasted measurement with the line measurement]\label{lem:h-b-consistency}
  \lean{MIPStarRE.Paper2009LDT.Section12Pasting.hBConsistency}
...
\begin{lemma}[Summing over all outcomes]\label{lem:over-all-outcomes}
  \lean{MIPStarRE.Paper2009LDT.Section12Pasting.overAllOutcomes}
...
\end{lemma}
\begin{proof}
  The definition of $H$ only sums over tuples compatible with a global polynomial. Insert the line measurement $B^u$, use Lemma~\ref{lem:h-b-consistency} to show that globally inconsistent tuples contribute little, and then replace distinct tuples by independent tuples with Lemma~\ref{prop:ld-dnoteq}.
\end{proof}

\begin{lemma}[From the pasted sum to a polynomial in $G$]\label{lem:from-H-to-G}
  \lean{MIPStarRE.Paper2009LDT.Section12Pasting.fromHToG}
  \uses{cor:G-hat-facts, lem:commute-g-half-sandwich, def:pasted-measurement}
  If $x_1,\dots,x_k$ are sampled independently and uniformly from $\F_q$, then
  \[
    \E_{x_1,\dots,x_k} \sum_{\tau\, :\, |\tau| \ge d+1} \sum_{(g_1,\dots,g_k) \in \mathsf{Outcomes}_\tau}
    \bra{\psi} \widehat H_{g_1,\dots,g_k}^{x_1,\dots,x_k} \ot I \ket{\psi}
...
\end{lemma}
\begin{proof}
  Peel off the $k$ completed measurements one at a time. The recurrence for the remaining type sum produces either a factor of $G$ or a factor of $I-G$, and Lemma~\ref{lem:commute-g-half-sandwich} controls the commutations needed to expose that recurrence.
\end{proof}

\criticize{`Section12Pasting.chernoffBernoulliMatrix` is now a named theorem interface, but no current file derives this bound from a spectral theorem or scalar concentration result. This remains one of the cleanest standalone milestones still missing from the Lean side.}{3}{5}

\begin{lemma}[Matrix Chernoff bound for the Bernoulli polynomial]\label{lem:chernoff-bernoulli-matrix}
  \lean{MIPStarRE.Paper2009LDT.Section12Pasting.chernoffBernoulliMatrix}
...
\begin{lemma}[Completeness of the pasted submeasurement]\label{cor:ld-pasting-N-completeness}
  \lean{MIPStarRE.Paper2009LDT.Section12Pasting.ldPastingNCompleteness}
