...
\criticize{The commutativity files now expose named output packages in `Section10CommutativityPoints.lean` and `Section11Commutativity.lean`, but the actual commutator estimates are still abstract fields rather than operator formulas. So this chapter's dependency shape is present without a genuine implemented commutation calculus yet.}{3}{5}
...
\begin{theorem}[Commutativity of the point measurements]\label{thm:commutativity-points}
  \lean{MIPStarRE.Paper2009LDT.Section10CommutativityPoints.commutativityPoints}
...
\begin{lemma}[Commutativity after evaluation]\label{lem:comm-data-processed-g}
  \lean{MIPStarRE.Paper2009LDT.Section11Commutativity.commDataProcessedG}
...
\end{lemma}
\begin{proof}
  Replace each evaluated $G$ by the corresponding point measurement using the consistency and boundedness hypotheses. The point measurements commute by Theorem~\ref{thm:commutativity-points}. Transfer the commutation estimate back to the evaluated $G$'s with the inner-product comparison lemma.
\end{proof}

\begin{lemma}[Normalization condition for a sandwiched family]\label{lem:normalization-condition}
  \lean{MIPStarRE.Paper2009LDT.Section11Commutativity.normalizationCondition}
...
\begin{theorem}[Commutativity of the slice measurements]\label{thm:com-main}
  \lean{MIPStarRE.Paper2009LDT.Section11Commutativity.comMain}
...
\end{theorem}
\begin{proof}
  Expand the commutator norm into two quartic terms. The first is compared with $\bra{\psi} G \ot G \ket{\psi}$ by Lemma~\ref{prop:switch-sandwich}. For the second, evaluate both slice measurements at random points, use Lemma~\ref{lem:comm-data-processed-g} together with the normalization condition from Lemma~\ref{lem:normalization-condition}, and then remove the evaluations by Schwartz--Zippel.
\end{proof}