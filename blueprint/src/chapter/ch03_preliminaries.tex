...
\begin{proposition}\label{prop:fourier-fact-scalar}
  Let $a \in \F_q$. Then
  \begin{equation*}
...
\begin{proposition}\label{prop:fourier-fact-vector}
  \uses{prop:fourier-fact-scalar}
  Let $v \in \F_q^m$. Then
  \begin{equation*}
...
\begin{proposition}\label{prop:cab-approx-delta}
  \uses{def:approx_delta}
...
\begin{lemma}[Completing a close submeasurement to a full measurement]\label{prop:completing-to-measurement}
  ...
  \uses{def:strong-self-consistency, def:approx_delta, prop:triangle-inequality-for-vectors-squared, prop:easy-approx-from-approx-delta, prop:cool-prop}
...
\begin{proof}
  ...
  \begin{align}
    ...
    &= \sum_a \bra{\psi} (A_a)^2 \ot I \ket{\psi}.
    \label{eq:As-on-same-side-rewrite}
  \end{align}
  Combine \eqref{eq:As-on-same-side-rewrite} with the strong self-consistency inequality.
\end{proof}
