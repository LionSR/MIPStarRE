...
\criticize{The active Lean scaffold still models the ambient finite field too coarsely. In `Section3Test.lean`, `Fq` is only `Fin q`, and the polynomial-like outcome types are plain function types, so honest field structure and individual-degree constraints are still missing.}{4}{5}
...
\begin{definition}[Post-processing]\label{def:post-processing}
  \lean{MIPStarRE.Paper2009LDT.Section4Preliminaries.postProcessing}
...
\begin{definition}[Completion of a submeasurement]\label{def:measurement-completion}
  \lean{MIPStarRE.Paper2009LDT.Section4Preliminaries.measurementCompletion}
  \uses{def:submeasurement}
...
\criticize{`Section4Preliminaries.lean` now exposes named output packages for this comparison calculus, but the underlying relations `ConsistencyRel`, `StateDependentDistanceRel`, and `StrongSelfConsistencyRel` in `Section3Test.lean` are still abstract placeholders and are not yet tied to the honest matrix-level infrastructure in `MIPStarRE/Quantum/Measurement.lean` and `MIPStarRE/Quantum/OutcomeFamily.lean`.}{4}{5}

\begin{definition}[Consistency]\label{def:simeq}
  \lean{MIPStarRE.Paper2009LDT.Section4Preliminaries.consistency}
...
\begin{lemma}[Consistency for measurements]\label{prop:simeq-for-measurements}
  \lean{MIPStarRE.Paper2009LDT.Section4Preliminaries.simeqForMeasurements}
  \uses{def:simeq}
...
\begin{definition}[State-dependent distance]\label{def:approx_delta}
  \lean{MIPStarRE.Paper2009LDT.Section4Preliminaries.stateDependentDistance}
...
\begin{lemma}[Consistency implies state-dependent distance for measurements]\label{prop:simeq-to-approx}
  \lean{MIPStarRE.Paper2009LDT.Section4Preliminaries.simeqToApprox}
  \uses{def:simeq, def:approx_delta, prop:simeq-for-measurements}
...
\begin{lemma}[Data processing for consistency]\label{prop:simeq-data-processing}
  \lean{MIPStarRE.Paper2009LDT.Section4Preliminaries.simeqDataProcessing}
  \uses{def:post-processing, def:simeq}
  If $A_a^x \ot I \simeq_\delta I \ot B_a^x$ and $f$ is a map on the answer set, then
  \[
...
\begin{lemma}[Consistency controls a submeasurement]\label{prop:cons-sub-meas}
  \lean{MIPStarRE.Paper2009LDT.Section4Preliminaries.consSubMeas}
...
\begin{lemma}[Switch-sandwich]\label{prop:switch-sandwich}
  \lean{MIPStarRE.Paper2009LDT.Section4Preliminaries.switchSandwich}
  \uses{def:approx_delta}
...
\begin{lemma}[Completeness transfer to a projective approximation]\label{prop:completeness-transfer-projective-P}
  \lean{MIPStarRE.Paper2009LDT.Section4Preliminaries.completenessTransferProjectiveP}
  \uses{def:approx_delta}
...
\begin{definition}[Strong self-consistency]\label{def:strong-self-consistency}
  \lean{MIPStarRE.Paper2009LDT.Section4Preliminaries.strongSelfConsistency}
...
\begin{lemma}[Strong self-consistency and state-dependent distance]\label{prop:two-notions-of-self-consistency}
  \lean{MIPStarRE.Paper2009LDT.Section4Preliminaries.twoNotionsOfSelfConsistency}
...
\begin{lemma}[Completing a close submeasurement to a full measurement]\label{prop:completing-to-measurement}
  \lean{MIPStarRE.Paper2009LDT.Section4Preliminaries.completingToMeasurement}
  \uses{def:strong-self-consistency, def:approx_delta, prop:triangle-inequality-for-vectors-squared}
...
\end{lemma}
\begin{proof}
  The squared triangle inequality reduces the problem to bounding the missing mass of $B$. Strong self-consistency of $A$ and the closeness of $B$ to $A$ show that this missing mass is small.
\end{proof}