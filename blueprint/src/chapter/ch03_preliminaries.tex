...
\section{Finite fields and Fourier orthogonality}


\begin{definition}[Finite field trace]\label{def:finite-field-trace}
  \lean{MIPStarRE.LDT.Preliminaries.ffTrace}
  \leanok
  Let $\F_q$ be a finite field with $q=p^t$ and let $\F_p$ be its prime subfield. The finite field trace is the function $\tr\colon \F_q \to \F_p$ defined by
  \[
    \tr[x] = \sum_{\ell=0}^{t-1} x^{p^\ell}.
  \]
\end{definition}

\begin{theorem}[Fourier orthogonality over a finite field]\label{prop:fourier-fact-scalar}
  \lean{MIPStarRE.LDT.Preliminaries.fourier_fact_scalar}
  \leanok
  \uses{def:finite-field-trace}
  Let $a \in \F_q$. Then
  \[
...
\end{theorem}
\begin{proof}
  If $a = 0$, then $\tr[\bx \cdot a] = 0$ for every $\bx \in \F_q$, so the expectation is $1$. If $a \neq 0$, choose $y \in \F_q$ such that $\tr[a \cdot y] \neq 0$. Translating $\bx$ by $y$ shows that the expectation is unchanged after multiplication by the nontrivial phase $\omega^{\tr[y \cdot a]}$, hence it must vanish.
\end{proof}

\begin{theorem}[Fourier orthogonality over $\F_q^m$]\label{prop:fourier-fact-vector}
...
\end{theorem}
\begin{proof}
  Apply the scalar orthogonality relation coordinatewise to the additive character $\bu \mapsto \omega^{\tr[\bu \cdot v]}$. The expectation factors over the coordinates, so it is $1$ exactly when every coordinate of $v$ vanishes and $0$ otherwise.
\end{proof}
...
\begin{definition}[Post-processing]\label{def:post-processing}
  \lean{MIPStarRE.LDT.Preliminaries.postProcessing}
...
\begin{definition}[Completion of a submeasurement]\label{def:measurement-completion}
  \lean{MIPStarRE.LDT.Preliminaries.measurementCompletion}
  \uses{def:submeasurement}
...
\begin{definition}[Consistency]\label{def:simeq}
  \lean{MIPStarRE.LDT.Preliminaries.consistency}
...
\begin{definition}[State-dependent distance]\label{def:approx_delta}
  \lean{MIPStarRE.LDT.Preliminaries.stateDependentDistance}
...
\begin{definition}[Strong self-consistency]\label{def:strong-self-consistency}
  \lean{MIPStarRE.LDT.Preliminaries.strongSelfConsistency}
