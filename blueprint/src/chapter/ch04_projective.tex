...
\criticize{`Section5MakingMeasurementsProjective.lean` now records the displayed quarter-power error terms explicitly, but `NaimarkStatement` still only packages correlation preservation abstractly, and the orthonormalization helper chain is still a naming scaffold rather than a formal spectral-truncation / SVD development.}{3}{5}
...
\end{lemma}
\begin{proof}
  Follow the Kempe--Vidick orthogonalization argument~\cite{KV11}. The proof first shows that a consistent measurement is almost projective, then rounds it to nearby projections while controlling the loss in overlap. In the source this compression hides several intermediate operator inequalities and norm-comparison steps; during Lean proof filling these are expected to appear as explicit helper lemmas (currently scaffolded as \lean{MIPStarRE.Paper2009LDT.Section5MakingMeasurementsProjective.consistencyToAlmostProjective} and \lean{MIPStarRE.Paper2009LDT.Section5MakingMeasurementsProjective.roundAlmostProjectiveMeasurement}) rather than a single black-box step.
\end{proof}
