...
\begin{lemma}[Bipartite strategy block identities]\label{lem:bipartite-strategy-block-identities}
	\lean{MIPStarRE.LDT.BiProjStrat.payloadBlock_nonneg, MIPStarRE.LDT.BiProjStrat.roleBlock_mul, MIPStarRE.LDT.BiProjStrat.roleBlock_nonneg}
...
\begin{remark}[$k$-bound boundary for the public theorem]\label{rem:main-formal-k-boundary}
	...
	pasting-side wrappers (see the statement of Theorem~\ref{thm:ld-pasting} in
	Chapter~7 of \cite{Ji2020LowIndividualDegree}). The Lean assembly of
	Theorem~\ref{thm:main-formal} reconciles the two by a case split on $k$:
	\begin{itemize}
		\item In the regime $400\,md \le k$, the induction and pasting wrappers
		      ...
		      Theorem~\ref{thm:error-cascade-main-formal}.
		\item In the regime $md \le k < 400\,md$, the bound
		      $1 \le \mainFormalError$ already holds (the paper's standard
		      observation from the proof of Theorem~\ref{thm:main-induction}, where
