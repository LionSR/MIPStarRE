...
\begin{definition}[General projective strategy]\label{def:projective-strategy}
	...
	% the current role-register symmetrization construction. Remaining follow-up work:
...
\begin{theorem}[Current same-space formal interface]\label{thm:main-formal-current-interface}
	...
	\leanok
	\uses{def:low-individual-degree-test, def:projective-strategy, def:polymeasurements, def:simeq, thm:main-formal}
...
\begin{remark}[$k$-bound boundary for the public theorem]\label{rem:main-formal-k-boundary}
	...
	The full source-boundary reduction is named in Lean as
...
\begin{remark}[Formal-interface boundary for the public theorem]\label{rem:main-formal-interface-boundary}
	...
	role-register symmetrization construction showing that the paper's general strategy
	reduces to the current formal interface.  In addition, the proof of the
	current same-space interface still runs through the Section~6 successor
	construction tracked by issue~\#1507, so this interface is not marked with
