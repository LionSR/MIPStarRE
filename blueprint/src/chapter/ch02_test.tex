...
\begin{remark}[Formal-interface boundary for the public theorem]\label{rem:main-formal-interface-boundary}
	...
	documented in
	\texttt{docs/paper-gaps/issue-930-main-formal-interface-restrictions.tex}.
	The source theorem, Theorem~\ref{thm:main-formal}, is therefore not marked as
	formalized.  Removing this restriction requires a two-space role-register
	symmetrization wrapper showing that the paper's general strategy reduces to
	the current formal interface.  In addition, the proof of the current
	same-space interface still runs through the Section~6 successor construction
	tracked by issue~\#1507, so this interface is not marked with proof-level
