...
\begin{proof}
	...
	\texttt{docs/paper-gaps/issue-906-main-formal-k-bound.tex} and
...
\end{proposition}
\begin{proof}\leanok
	\proves{prop:main-formal-source-obligation}
	...
	Proposition~\ref{prop:main-formal-source-small-error-obligation}.
\end{proof}
...
\end{proposition}
\begin{proof}\leanok
	\proves{prop:main-formal-source-small-error-obligation}
	The proof is the theorem
	\texttt{mainFormal\_sourceConclusion\_ofRoleRegisterScalarBoundary}, which
	applies the checked two-space role-register construction and then absorbs its
	explicit errors into \(\nu\).
\end{proof}
...
\begin{proposition}[Two-space role-register reduction for the source theorem]\label{prop:main-formal-source-two-space-role-register}
	...
	hypothesis \(k\ge 400md\), and the resulting role-register consistency
	estimate can be unsymmetrized into the two point-consistency estimates on the
	original two-space strategy with the expected factor \(2\).  In addition, the
	...
	the same-space carrier to a bipartite state on \(H_A\otimes H_B\).  The
		...
		The combined output is the small-error branch used in the corrected
		two-space source theorem, where \(k>0\) is now part of the theorem
		statement.
\end{proposition}
\begin{proof}
...
\begin{remark}[$k$-bound boundary for the public theorem]\label{rem:main-formal-k-boundary}
	...
	saturated-error branch and delegates the non-vacuous branch to the checked
	\texttt{MIPStarRE.LDT.Test.mainFormal\_sourceSmallErrorObligation}; neither
	declaration is an extra hypothesis of Theorem~\ref{thm:main-formal}.
\end{remark}
...
\begin{remark}[Formal-interface boundary for the public theorem]\label{rem:main-formal-interface-boundary}
	...
	source-shaped Lean statement and is proof-complete under the corrected
	boundary assumptions \(k\ge 400md\) and \(k>0\).  The current same-space
	corrected-range interface remains a Lean-only intermediate theorem; it is
	also proof-complete, but it is not the printed two-space source theorem.
\end{remark}
