...
\begin{proof}
	\leanok
	\uses{prop:main-formal-source-reduction}
...
\begin{proposition}[Source-boundary reduction for the final theorem]\label{prop:main-formal-source-reduction}
	\lean{MIPStarRE.LDT.Test.mainFormal_sourceConclusion}
	\leanok
...
\begin{proposition}[Small-error source-boundary conclusion for the final theorem]\label{prop:main-formal-source-small-error}
	\lean{MIPStarRE.LDT.Test.mainFormal_sourceSmallErrorConclusion}
	\leanok
...
\begin{proposition}[Two-space role-register reduction for the source theorem]\label{prop:main-formal-source-two-space-role-register}
	...
	unsymmetrizes the polynomial measurement.  Lean now formalizes this reduction
	for the general two-space source strategy rather than assuming it as an
	interface restriction.  The trace-level factor-two unsymmetrization estimate
	for arbitrary heterogeneous role-register measurements is formalized in
...
\begin{proof}
	...
	\texttt{mainFormal\_sourceConclusion\_ofRoleRegisterScalarBoundary}.  It
	combines the source-shaped main-induction theorem, the heterogeneous
	unsymmetrization estimates, projectivization on both local Hilbert spaces, the
	completion and line-169 consistency steps, and the final scalar absorption
	under the nonzero sampling boundary.
\end{proof}
...
\begin{theorem}[Current same-space formal interface]\label{thm:main-formal-current-interface}
	...
	used by the present role-register symmetrization construction.  The corrected
	two-space source theorem is recorded above; the literal printed interface
	still differs by the large-\(k\) and nonzero-sampling corrections.
	Let $(\psi, A^{\mathrm A}, B^{\mathrm A}, L^{\mathrm A},
