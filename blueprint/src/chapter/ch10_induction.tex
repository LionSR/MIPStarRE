...
\begin{proof}
  ...
  Complete the input submeasurement $G$ by adjoining the missing mass as an extra outcome. Apply Theorem~\ref{thm:self-improvement} to the resulting measurement, then restrict back to the original outcome set and transfer the consistency and boundedness estimates through completion and data processing. The source keeps this stronger submeasurement-input form explicit, so the blueprint keeps a separate node for it instead of identifying it with Theorem~\ref{thm:self-improvement}.
\end{proof}
