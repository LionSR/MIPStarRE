...
Do not fix review comments by strengthening a source-labelled theorem statement.
In particular, do not add bridge, residual, repair, producer, package, or
arbitrary implication hypotheses to a theorem that is meant to match a paper
statement. If such data is already present, treat it as proof debt: extract any
usable proof content into source-faithful lemmas, or report the existing issue
that tracks the missing producer. If the proof cannot be closed from the paper
hypotheses, stop and comment instead of pushing a conditional theorem.
...
`references/ldt-paper/` where the original LDT theorem statements and proofs are
stored — read the relevant sections, compare hypotheses and conclusions, and cite
the specific paper path, line, and label when flagging a discrepancy.
