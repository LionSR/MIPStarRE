...
Sandwich-tensor expansion lemmas, collision residuals, tensor
marginalization bounds (x-side and y-side), normalization conditions
and self-consistency bounds used by the scalar-to-tensor bridges.
