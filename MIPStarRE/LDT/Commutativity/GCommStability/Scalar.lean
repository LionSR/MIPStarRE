...
Paper-faithful scalar stability bounds derived from the overlap stability submodules.
-/
...
private lemma gCommStability_scalar_pointwise_bound
  ...
  have hsecond :
    ...
    have hop_mono_right :
      ...
      have hpsd : Matrix.PosSemidef (Matrix.kronecker A (B₂ - B₁)) :=
        Matrix.nonneg_iff_posSemidef.mp <|
    ...
    calc
      ...
      _ ≤ ev strategy.state
            ...
            exact opTensor_mono_left
              (by
                calc
                  ((1 - T) * R.total * (1 - T)) ≤ (1 - T) * 1 * (1 - T) :=
                    MIPStarRE.Quantum.sandwich_mono hTc_herm R.total_le_one
                  _ = 1 - T := by
                    calc
