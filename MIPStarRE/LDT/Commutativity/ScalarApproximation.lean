...
/-- The paper's slice submeasurement `R^y_g = E_{u,x} ∑_a G^{u,x}_a G^y_g G^{u,x}_a`. -/
...
private noncomputable def gCommStabilityBoundedResidual
    (params : Parameters)
    [FieldModel params.q]
    (strategy : SymStrat params.next ι)
...
private theorem gCommStability_storedBoundedResidualBound
    (params : Parameters)
    [FieldModel params.q]
    (strategy : SymStrat params.next ι)
    (zeta : Error)
...
private theorem gCommStabilityBoundedResidual_nonneg
    (params : Parameters)
    [FieldModel params.q]
    (strategy : SymStrat params.next ι)
    (family : IdxPolyFamily params ι)
...
private theorem gCommStability_averagedPoint_le_witness
    (params : Parameters)
    [FieldModel params.q]
    (strategy : SymStrat params.next ι)
    (zeta : Error)
    ...
    calc
      ...
      _ = opTensor (CFC.sqrt (R.outcome g)) (1 : MIPStarRE.Quantum.Op ι) *
            opTensor (CFC.sqrt (R.outcome g)) (1 : MIPStarRE.Quantum.Op ι) := by
              ...
              simp [hsqrt_herm]
      _ = opTensor (CFC.sqrt (R.outcome g) * CFC.sqrt (R.outcome g))
...
set_option maxHeartbeats 2000000 in
private theorem gCommStability_scalar
    (params : Parameters)
    [FieldModel params.q]
    (strategy : SymStrat params.next ι)
    (zeta : Error)
