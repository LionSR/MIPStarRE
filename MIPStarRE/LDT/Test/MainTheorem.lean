...
/-- Wrapper exposing the current bridge-backed Section 3 endpoint.

This theorem is intentionally separate from `mainFormal` so the exported
statement of `thm:main-formal` is not weakened while the upstream assembly is
still under formalization. -/
theorem mainFormal_of_bridge
