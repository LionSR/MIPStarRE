...
The top-level conditional bridge for `mainFormal` uses this constructor only for
the checked base-case role residual.  Successor-branch bridge data is kept in the
separate successor residual obligations.
...
opaque mainFormalBaseBranchRepairedBridgeProducer
  ...
  -- paper's base-case orthonormalization and completion argument, using the checked
  -- base-case role residual rather than assuming this input in `mainFormal`.
  sorry
