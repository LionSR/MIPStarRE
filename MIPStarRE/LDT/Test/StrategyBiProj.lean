...
paper-faithful two-space container `ProjStrat params ι ι`.

Discards the swap-symmetry data (`permInvState`, `densityFixed`) since
`ProjStrat` does not carry same-space swap assumptions by design. -/
def toProjStrat {params : Parameters} [FieldModel params.q]
    {ι : Type*} [Fintype ι] [DecidableEq ι]
    (strategy : ProjStrat params ι) : ProjStrat params ι ι where
