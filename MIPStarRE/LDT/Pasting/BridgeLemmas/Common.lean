...
/-- If the density matrix is invariant under swapping the two tensor factors,
then expectation is invariant under swapping the tested observable. -/
lemma ev_swapDensity_of_density_fixed
    (ψ : QuantumState (ι × ι))
...
/-- For a swap-invariant bipartite state, the expectation of `X ⊗ Y` is equal
to the expectation of `Y ⊗ X`. -/
lemma ev_opTensor_swap_of_density_fixed
    (ψ : QuantumState (ι × ι))
    (hfix : swapDensity ψ.density = ψ.density)
...
/-- For a swap-invariant bipartite state, the bipartite matching mass is
symmetric in the two submeasurements. -/
lemma qBipartiteMatchMass_symm_of_density_fixed
    (ψ : QuantumState (ι × ι))
    (hfix : swapDensity ψ.density = ψ.density)
    {Outcome : Type*} [Fintype Outcome]
    (A B : SubMeas Outcome ι) :
...
/-- For a swap-invariant bipartite state, the pointwise bipartite consistency
defect is symmetric in the two submeasurements. -/
lemma qBipartiteConsDefect_symm_of_density_fixed
    (ψ : QuantumState (ι × ι))
    (hfix : swapDensity ψ.density = ψ.density)
    {Outcome : Type*} [Fintype Outcome]
    (A B : SubMeas Outcome ι) :
...
/-- For a swap-invariant bipartite state, a consistency estimate is symmetric
in the two indexed submeasurements. -/
lemma consRel_symm_of_density_fixed
    (ψ : QuantumState (ι × ι))
    (hfix : swapDensity ψ.density = ψ.density)
