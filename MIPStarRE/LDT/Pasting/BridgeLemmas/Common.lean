...
/-!
# Section 12 pasting: bridge common lemmas

Shared helper lemmas used by the bridge proofs for the sandwich, consistency, and final pasting statements.
-/
...
lemma polynomialEvaluationFamily_constructedPastedSubMeas
    (params : Parameters) [FieldModel params.q]
    ...
    polynomialEvaluationFamily params.next (constructedPastedSubMeas params family k) =
      fun u =>
        averageIdxSubMeas
          ...
          (fun xs => evaluateAt params.next u (pastedInterpolationFamily params family k xs))
          (distinctTupleDistribution_weight_sum_le_one params k) := by
  funext u
  simpa [constructedPastedSubMeas, polynomialEvaluationFamily] using
    evaluateAt_averageIdxSubMeas (params := params.next) u
      (distinctTupleDistribution params k)
...
lemma avgOver_uniform_indicator_le_avgOver_distinct_add_tv
    (params : Parameters) [FieldModel params.q]
    (k : ℕ) (hk : k ≤ params.q)
  ...
  have htv_eq :
    ...
    simp [hgood, hbad]
    ring
  have hsupport_term_le :
      ∑ xs ∈ support,
          (uniformDistribution (PointTuple params k)).weight xs *
            (if P xs then (1 : Error) else 0)
        ≤ avgOver (distinctTupleDistribution params k)
    ...
    calc
      ∑ xs ∈ support,
        ...
        ≤ ∑ xs ∈ support,
            (distinctTupleDistribution params k).weight xs *
              (if P xs then (1 : Error) else 0) := by
              refine Finset.sum_le_sum ?_
              intro xs hxs
              have hind_nonneg : 0 ≤ (if P xs then (1 : Error) else 0) := by
              ...
              have hw :
                ...
                rw [if_pos hxs]
                exact hweight_le
              exact mul_le_mul_of_nonneg_right hw hind_nonneg
  ...
  have hbad_term_le :
      ∑ xs ∈ bad,
          (uniformDistribution (PointTuple params k)).weight xs *
            (if P xs then (1 : Error) else 0)
        ≤ ∑ xs ∈ bad, (uniformDistribution (PointTuple params k)).weight xs := by
    refine Finset.sum_le_sum ?_
    intro xs hxs
    by_cases hPx : P xs
    · simp [hPx]
    · have hnonneg : 0 ≤ (uniformDistribution (PointTuple params k)).weight xs := by
        exact (uniformDistribution (PointTuple params k)).nonnegative xs
      simp [hPx, hnonneg]
  calc
    avgOver (uniformDistribution (PointTuple params k))
        (fun xs => if P xs then (1 : Error) else 0)
    ...
    _ ≤ avgOver (distinctTupleDistribution params k)
          (fun xs => if P xs then (1 : Error) else 0)
        + ∑ xs ∈ bad, (uniformDistribution (PointTuple params k)).weight xs := by
            exact add_le_add hsupport_term_le hbad_term_le
    _ = avgOver (distinctTupleDistribution params k)
          (fun xs => if P xs then (1 : Error) else 0)
        + totalVariationDistance
            (uniformDistribution (PointTuple params k))
