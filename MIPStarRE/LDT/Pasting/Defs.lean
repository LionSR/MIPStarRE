...
/-- The subset `\mathsf{Global}_\tau(x)` of `\mathsf{Outcomes}_\tau`
consisting of tuples that arise from restrictions of a single global
polynomial at the slice heights `xs`. -/
def globallyConsistentOutcomesByType (params : Parameters)
    [FieldModel params.q]
    {k : ℕ} (xs : PointTuple params k) (τ : GHatType k) :
    Set (GHatTupleOutcome params k) :=
  { gs | gs ∈ outcomesByType τ ∧
    IsGloballyConsistent params xs gs }

/-- The complement `\overline{\mathsf{Global}_\tau(x)}` inside
`\mathsf{Outcomes}_\tau`. -/
def nonglobalOutcomesByType (params : Parameters)
    [FieldModel params.q]
    {k : ℕ} (xs : PointTuple params k) (τ : GHatType k) :
    Set (GHatTupleOutcome params k) :=
  outcomesByType τ \
    globallyConsistentOutcomesByType params xs τ

/-- Interpolate from `d+1` or more genuine slice polynomials
to recover a polynomial in `m+1` variables via Lagrange
interpolation.
