...
theorem ldGbcon
  ...
  have hpoint_to_axis := pointVerticalLineSdd params strategy eps delta gamma hgood
  have htriangle :=
    Preliminaries.triangleSub_right strategy.state
      ...
      (evaluateFiberFamilyAtNextPoint params
        (IdxProjSubMeas.toIdxSubMeas family.meas))
      pointMeas
      (ldGbconVerticalLineMeasurement params strategy)
      zeta
      (8 * (params.m : Error) * eps + 4 * delta)
      hcons_swapped
      hpoint_to_axis
  simpa [ldGbconVerticalLineMeasurement] using htriangle
