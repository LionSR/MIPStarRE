...
/-!
# Section 12 pasting: from-H-to-G adjacent-stage chain

Definitions for the paper's adjacent-stage chain `M₁ → M₂ → M₃ → M₄` and the
half-sandwich and final-move rewrite lemmas connecting successive scalars.

The source scalars `A₀`, `M₁` and the supporting context algebra are defined in
`AdjacentStages.StageA0M1`.
-/
