...
- `SpectralTruncationStatement.toRoundingToProjectorsWitness` — the reverse
  field-for-field conversion used by the QXP rank-reduction layer
- `projectiveLowRankSum_of_spectralTruncationStatement` and
  `projectiveLowRankSum_of_spectralTruncationInput` — bridges from the
  orthonormalization spectral interface to the paper's QXP rank-reduction
  witness, with the source almost-projectivity bound kept explicit
...
- `projectiveNonMeasurement_of_sourceAlmostProjective` — constructive
  spectral-truncation witness from the paper's almost-projective hypothesis
- `spectralTruncationInput_of_sourceAlmostProjective` — the same constructive
  producer packaged directly as `SpectralTruncationInput`
...
private lemma roundedProjectorFamily_closeness {Outcome : Type uOutcome}
  ...
  constructor
  have hq :
      qSDDOp ψ (A.toSubMeas : OpFamily Outcome ι) (roundedProjectorFamily A δ) ≤ (1 / δ) * ζ := by
...
private lemma roundedProjectorFamily_total_le_one_zero {Outcome : Type uOutcome}
  ...
  have houtcome_le :
    ...
    intro a
    have hmono :
        cfc (truncationCutoff 0) (A.outcome a) ≤ cfc (R := ℝ) (id : Error → Error) (A.outcome a) := by
      apply cfc_mono (R := ℝ) (a := A.outcome a)
      ...
      have hterm_le :
        ...
        have hratio_nonneg : ∀ i : ι, 0 ≤ ratio i := by
          ...
          · have hx0 : 0 ≤ hherm.eigenvalues i :=
              outcome_spectrum_nonneg A a (hherm.eigenvalues i) (hherm.eigenvalues_mem_spectrum_real i)
            have hx1 : hherm.eigenvalues i ≤ 1 :=
              outcome_spectrum_le_one A a (hherm.eigenvalues i) (hherm.eigenvalues_mem_spectrum_real i)
        ...
        have hgap_le :
          ...
          have hratio_le : ratio i ≤ K := by
            dsimp [K]
            simpa using Finset.single_le_sum (fun j _ => hratio_nonneg j) (by simp : i ∈ Finset.univ)
          ...
          calc
            cfc gap (A.outcome a)
              = cfc (fun x : Error => x - truncationCutoff 0 x) (A.outcome a) ^ 2 := by
                  exact cfc_pow (R := ℝ) (fun x : Error => x - truncationCutoff 0 x) 2 (A.outcome a)
                    (hf := continuousOn_outcome_spectrum A a (fun x : Error => x - truncationCutoff 0 x))
                    (ha := hsa)
          ...
          have hsub :
            ...
            calc
              cfc defect (A.outcome a)
                = cfc id (A.outcome a) - cfc (fun x : Error => x ^ (2 : Nat)) (A.outcome a) := by
                    simpa using
                      (cfc_sub (R := ℝ) (f := id) (g := fun x : Error => x ^ (2 : Nat)) (a := A.outcome a)
                        (hf := continuousOn_outcome_spectrum A a id)
                        (hg := continuousOn_outcome_spectrum A a (fun x : Error => x ^ (2 : Nat))))
              _ = A.outcome a - cfc (fun x : Error => x ^ (2 : Nat)) (A.outcome a) := by
                    simpa using congrArg
...
theorem projectiveNonMeasurement_of_sourceAlmostProjective_full
  ...
  · by_cases hsmall : ζ ≤ 1 / 4
    ...
    · exact projectiveNonMeasurement_of_sourceAlmostProjective_large ψ A ζ hψ (lt_of_not_ge hsmall)
