...
This declaration now records the honest output statement consumed by the QXP
rank-reduction layer: a chosen family `R_a` equipped with
`RoundingToProjectorsWitness ψ A ζ R`. The old
`ProjectiveNonMeasurementBridgePackage` placeholder has been deleted; producing
such a witness from `eq:A-looks-projective` remains an upstream
spectral-truncation obligation rather than a downstream QXP bridge. -/
