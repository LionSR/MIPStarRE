...
`localOrthonormalization_ofInternalInputs`, the only remaining external inputs are the
    ...
    have hAhatssc :
      ...
      simpa [Ahat] using
        Orthonormalization.Completion.optionCompletion_bipartiteSSCRel
          ...
          (hψ := hψ) (A := A) (ζ := ζ) hssc
    obtain ⟨P, hP⟩ :=
      localOrthonormalization_ofInternalInputs (Outcome := Option Outcome) (ι := ι)
